...
class VggBlock(nn.Module):
    def __init__(self, input_channels, output_channels, repetitions=2):
        ...
        self.remaining_blocks = nn.Sequential(
            *[VggSubBlock(output_channels, output_channels) for idx in range(1, repetitions)])
...
class DecoderBlock(nn.Module):
    def __init__(self, input_channels, output_channels, repetitions=2):
        ...
        self.first_blocks = nn.Sequential(
            *[VggSubBlock(input_channels, input_channels) for idx in range(1, repetitions)])
...
class Decoder(nn.Module):
    def __init__(self, channels):
        super().__init__()
        self.max_uppool = nn.MaxUnpool2d(kernel_size=2, stride=2, padding=0)
    ...
    def forward(self, x, pool_indices):
        for idx, block in enumerate(self.decoder_blocks):
            x = self.max_uppool(x, pool_indices[idx])
            x = block(x)
        return x
...
class SegNet(nn.Module):
    def __init__(self, enc_chs, dec_chs, num_classes=2, warm_start=True):
        ...
        if warm_start:
            load_vgg_weights_to_encoder(self.encoder)
